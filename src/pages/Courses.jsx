                    ...
                    <ul className='flex flex-col'>
                        ...
                        <div className={isOpenLevel ? 'flex flex-wrap gap-2 px-6 py-3' : 'hidden'}>
                            {["Beginner", "Intermediate", "Advanced"].map((level) => {
                                return (
                                    <div onClick={() => { setLevelSelected(level) }} className={LevelSelected == level ? 'bg-[#9747FF] cursor-pointer  rounded-2xl px-4  text-white p-1' : 'border-2 border-[#999999] cursor-pointer  rounded-2xl px-4  text-[#999999] p-1'}>{level}</div>
                                )
                            })}
                        ...
                        <div className={isOpenCertf ? 'flex flex-wrap gap-2 px-6 py-3' : 'hidden'}>
                            {["Available", "Unavailable"].map((certf) => {
                                return (
                                    <div onClick={() => { setCertfSelected(certf) }} className={CertfSelected == certf ? 'bg-[#9747FF] cursor-pointer   rounded-2xl px-4  text-white p-1' : 'border-2 border-[#999999] cursor-pointer  rounded-2xl px-4  text-[#999999] p-1'}>{certf}</div>
                                )
                            })}
                        </div>
                  ...
                  onChange={(e) => setPriceRange({ ...priceRange, max: +e.target.value })} type="number" placeholder='Max' className="w-20 px-2 py-1 border-2 rounded-2xl outline-none focus:ring-2 focus:ring-[#9747FF] border-gray-300" /><span className='text-[#9747FF] text-xl absolute right-3'>$</span></span>
                        ...
                        <div className={isOpenDuration ? 'flex flex-wrap gap-2 px-6 py-3' : 'hidden'}>
                            {["<5h", ">5h and 10h<", ">10h"].map((duration) => {
                                return (
                                    <div onClick={() => { setDurationSelected(duration) }} className={DurationSelected == duration ? 'bg-[#9747FF] cursor-pointer  rounded-2xl px-4  text-white p-1' : 'border-2 border-[#999999] cursor-pointer  rounded-2xl px-4  text-[#999999] p-1'}>{duration}</div>
                                )
                            })}
                        ...
                        <div className={isOpenRating ? 'flex flex-wrap gap-2 py-2 px-6' : 'hidden'}>
                            {[1, 2, 3, 4, 5].map((rate) => {
                                return (
                                    <div onClick={() => { setRatingSelected(rate) }} className={RatingSelected == rate ? 'bg-[#9747FF] cursor-pointer  rounded-2xl px-4  text-white p-1' : 'border-2 border-[#999999] cursor-pointer  rounded-2xl px-4  text-[#999999] p-1'}>{rate!=5?`>${rate} star`:`${rate} star`} </div>
                                )
                            })}
                ...
                <div className='flex-none bg-white w-80 h-full shadow-xl'>
                    ...
                    <ul className='flex flex-col'>
                        {categories && categories.map((categorie, index) => {
                            return (
                                <li key={index} onClick={()=>{setCategorySelected(categorie.name);setSubCategorySelected("")}} className='pl-8 py-4 text-lg  group '>
